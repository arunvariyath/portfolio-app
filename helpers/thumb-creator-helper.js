...
module.exports = {
  createThumbnailImage(data) {
    ...
    Canvas.loadImage(imageLocation).then((image) => {
      ...
      Canvas.registerFont("public/fonts/Manjari-Bold.ttf", {
        family: "Manjari-Bold",
        weight: "bold",
      });
      Canvas.registerFont("public/fonts/Gayathri-Regular.ttf", {
        family: "Gayathri-Regular",
      });

      const width = image.width; // width of the image
      const height = image.height; // height of the image
      const poemTitleArray =
        data.poemTitle == undefined ? [] : data.poemTitle.split(" ");
      const canvasProps = {
        width: width,
        height: height,
      };
      ...
      // Draw the YT play button
      // context.drawImage(playBtn, 150, 200, 300, 200);
      console.log("lenth of title" + data.poemTitle.length);
      if (data.poemTitle.length <= 10) {
        // Draw the Title
        ...
        poemTitleArray.forEach((poemTitleword, index) => {
          ...
          if (index <= 1) drawText(context, modifiedMainTitleProps);
        });
      }
      ...
      singerImg.onload = () => {
        context.drawImage(
          ...
          singerPicProps.x,
          singerPicProps.y,
          singerPicProps.width,
          singerPicProps.height
        );
      };
      ...
      writerImg.onload = () => {
        ...
        context.drawImage(
          ...
          writerPicProps.x,
          writerPicProps.y,
          writerPicProps.width,
          writerPicProps.height
        );
      };
      ...
      fs.writeFileSync(
        "public/images/thumbnails/" + data.imageName + ".png",
        buffer
      );
    });
  },
